...
%> 
	<li>
		...
		<span class="col6"><%=sdf.format(article.getPubdate())%></span>	<!-- 출판일 -->
		<span class="col7"><%=article.getPage()%></span>		<!--  -->
		<span class="col8"><a href="filedownload.jsp?bimage=<%=article.getBimage()%>"><%=article.getBimage()%></a></span>		<!--  -->
		...
		<span class="col10"><button type="button" onclick="check(<%=article.getBnum()%>)">삭제</button></span>		<!-- db뿐만 아니라ㅣ 이미지도 삭제 -->
	</li>
<% 	} %>  
  </ul>	
  <br>
