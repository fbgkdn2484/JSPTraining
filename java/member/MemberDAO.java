...
public class MemberDAO {				//쿼리문 하나 마다 메서드 생성해서 사용

	private static MemberDAO instance = new MemberDAO();
	
	public static MemberDAO getInstance() {		//싱글턴 패턴 하나를 만들어서 공유함으로써, 오버헤드를 줄임
		return instance;
	}
	...
	private Connection getConnection() {
		...
		{
			System.out.println("booksite 데이터베이스 연결에 문제가 발생했습니다.");
			return null;
		}
	}
	...
	public int idCheck(String id) {	//전달받은 아이디와 패스워드가 테이블에 저장되어있는지 확인해주는 로직
		...
		try {
			...
			e.printStackTrace();
			System.out.println("member 테이블의 아이디 확인에 문제가 발생했습니다.");	//실행한 콘솔창에 드러남
	...
	public void insertMember(MemberVO ariicle) {
		...
		try{	
			...
			e.printStackTrace();
			System.out.println("member 테이블 삽입에 문제가 발생했습니다.");
		}
		finally{
			//5. 과부하가 걸리지 않도록 연결을 끊어주어야 한다. (자원 해제)
			/*
	...
	public int getArticleCount() {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("member 테이블의 인원 수 검색에 실패했습니다.");	//실행한 콘솔창에 드러남
	...
	public List<MemberVO> getArticles(int start, int end){			//자바 문법으로 여러개의 데이터를 가져올 때 사용하는 것 페이징 처리를 위해 매개변수 2개 가져옴 
		
		Connection conn = null;		//데이터베이스 연결 객체
		PreparedStatement pstmt = null;		//query 실행 객체
		ResultSet rs = null;		//select문 실행 결과를 받아오는(저장하는) 객체
		List<MemberVO> alist = null;		//초기값
		
		try {
			conn = getConnection();		//위의 getConnection()메서드에서 객체를 반환받아 값을 할당받아옴	DB연결
			
			String sql = "select * from member order by reg_date desc limit ?, ?";		//페이징 처리를 위해
			pstmt = conn.prepareStatement(sql);		//3. sql을 샐행하기 위한 객체를 생성
			...
			{//값이 있으면 참 없으면 거짓
				...
				alist = new ArrayList<MemberVO>(end);		//배열 형태로 사용	end를 지정함으로써 10개만 가져옴 가변은 오버헤드를 많이 차지하기 때문에
				do {				//먼저 실행하고 조건 판별
					MemberVO article = new MemberVO();		//같은 패키지 내에 있어서 객체 바로 사용 가능
			...
			e.printStackTrace();
			System.out.println("member 테이블의 자료 검색에 실패했습니다.");	//실행한 콘솔창에 드러남
	...
	//상세 글 보기
	public MemberVO getArticle(String id){			//하나만 전달 받기 때문에 List사용 x  상세보기를 클릭했다는 의미로 자동으로 조회수를 1 증가시킴
			
			Connection conn = null;		//데이터베이스 연결 객체
			PreparedStatement pstmt = null;		//query 실행 객체 쿼리문 실행을 위해 필요
			ResultSet rs = null;		//select문 실행 결과를 받아오는(저장하는) 객체
			MemberVO article = null;		//초기값
			
			try {
				conn = getConnection();		//위의 getConnection()메서드에서 객체를 반환받아 값을 할당받아옴	DB연결
				
				String sql = "select * from member where id = ?";		//sql문을 새로 선언하지 않고 사용할 수 있음 단, 밑에서 닫아주어야 한다.
				pstmt = conn.prepareStatement(sql);		//3. sql을 샐행하기 위한 객체를 생성
				...
				{				
					...
					article = new MemberVO();		//같은 패키지 내에 있어서 객체 바로 사용 가능
					article.setId(rs.getString("id"));
					article.setPasswd(rs.getString("passwd"));
					article.setName(rs.getString("name"));
					article.setReg_date(rs.getTimestamp("reg_date"));
	...
	//글 수정을 위한 상세보기
	public MemberVO getArticleUpdate(String id){			//하나만 전달 받기 때문에 Lisg사용 x  상세보기를 클릭했다는 의미로 자동으로 조회수를 1 증가시킴
	...
	public int updateMember(MemberVO article) {	//리턴 타입 int로 선언 article로 값을 전달 받음
	...
	public int deleteMember(String id) {	//오버로딩 이름이 같아도 메서드 전달받는 게 다르면 다른 걸로 인식
